...
import { ApiErrorCode, type ApiErrorCode as ApiErrorCodeValue } from './domain/constants';
...
function safeUnlink(assetDir: string, filename: string | null): void {
  ...
  try {
    unlinkSync(path.join(assetDir, filename));
  } catch {
...
function hasExactContainerLength(buffer: Buffer, format: string): boolean {
  if (format === 'jpeg')
    return (
      buffer.length >= 2 && buffer[buffer.length - 2] === 0xff && buffer[buffer.length - 1] === 0xd9
    );
...
async function validateImage(body: unknown, mime: string): Promise<Buffer> {
  ...
  try {
    ...
    if (
      ...
      pageHeight > MAX_LOGO_DIMENSION ||
      metadata.width * pageHeight * pages > MAX_LOGO_PIXELS
    ) {
      throw new EventAssetError(
        413,
        ApiErrorCode.ImageDimensionsExceeded,
        'Image dimensions exceed 4096 × 4096 limit',
      );
    }
...
export const parseEventLogo: RequestHandler = (req, res, next) => {
  ...
  express.raw({ type: () => true, limit: MAX_LOGO_BYTES })(req, res, error => {
    if (error && typeof error === 'object' && 'status' in error && error.status === 413) {
      next(new EventAssetError(413, ApiErrorCode.ImageTooLarge, 'Image exceeds 2 MiB limit'));
      return;
    }
    next(error);
  });
...
export async function storeEventLogo(
  runtime: AppRuntime,
  queue: QueueRow,
  body: unknown,
  ...
  try {
    ...
    const result = runtime.db
      .prepare('UPDATE queues SET logo_path=? WHERE id=? AND removed_at IS NULL AND logo_path IS ?')
      .run(filename, queue.id, queue.logo_path);
    if (result.changes !== 1) {
      const latest = runtime.db
        .prepare('SELECT removed_at FROM queues WHERE id=?')
        .get(queue.id) as { removed_at: string | null } | undefined;
      throw latest?.removed_at
        ? new EventAssetError(409, ApiErrorCode.EventArchived, 'Archived events cannot be edited')
        : new EventAssetError(
            409,
            ApiErrorCode.EventAssetConflict,
            'Event logo changed; reload and retry',
          );
    }
  } catch (error) {
...
export function removeEventLogo(runtime: AppRuntime, queue: QueueRow): boolean {
  if (!queue.logo_path) {
    const result = runtime.db
      .prepare(
        'UPDATE queues SET logo_path=NULL WHERE id=? AND removed_at IS NULL AND logo_path IS NULL',
      )
      .run(queue.id);
    if (result.changes === 1) return false;
    const latest = runtime.db.prepare('SELECT removed_at FROM queues WHERE id=?').get(queue.id) as
      { removed_at: string | null } | undefined;
    if (latest?.removed_at)
      throw new EventAssetError(
        409,
        ApiErrorCode.EventArchived,
  ...
  const result = runtime.db
    .prepare(
      'UPDATE queues SET logo_path=NULL WHERE id=? AND removed_at IS NULL AND logo_path IS ?',
    )
...
export function reconcileEventAssets(runtime: Pick<AppRuntime, 'assetDir' | 'db'>): void {
  const referenced = new Set(
    (
      runtime.db
        .prepare('SELECT logo_path FROM queues WHERE logo_path IS NOT NULL')
        .all() as Array<{ logo_path: string }>
    ).map(row => row.logo_path),
  );
  for (const filename of readdirSync(runtime.assetDir)) {
    if (
      temporaryAsset.test(filename) ||
      (generatedAsset.test(filename) && !referenced.has(filename))
    )
      safeUnlink(runtime.assetDir, filename);
  }
}
...
export function serveEventAsset(runtime: Pick<AppRuntime, 'assetDir' | 'db'>): RequestHandler {
  return (req, res, next) => {
    ...
    if (!filename || !generatedAsset.test(filename)) {
      notFound();
      return;
    }
    const referenced = runtime.db
      .prepare('SELECT 1 FROM queues WHERE logo_path=? LIMIT 1')
      .get(filename);
    if (!referenced) {
      notFound();
      return;
    ...
    res.sendFile(
      path.join(runtime.assetDir, filename),
      {
        dotfiles: 'deny',
      ...
      error => {
        ...
        if ((code === 'ENOENT' || code === 'ENOTDIR') && !res.headersSent) {
          notFound();
          return;
        }
        next(error);
