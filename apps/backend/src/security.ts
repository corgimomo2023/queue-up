...
import { Session } from './config/http-constants';

const VENDOR_ADMIN_SESSION_ROLE = 'super-admin';
...
export function verifySlowHash(value: string, encoded: string | null, normalize = true): boolean {
  ...
  try {
    ...
    const actual = crypto.scryptSync(
      normalize ? normalizeCredential(value) : value,
...
export function signStaffSession(queueId: number, secret: string): string {
  const payload = Buffer.from(
    JSON.stringify({ q: queueId, exp: Date.now() + Session.DurationMilliseconds }),
  ).toString('base64url');
...
export function signVendorAdminSession(secret: string): string {
  const payload = Buffer.from(
    JSON.stringify({
      role: VENDOR_ADMIN_SESSION_ROLE,
      exp: Date.now() + Session.DurationMilliseconds,
    }),
  ).toString('base64url');
  const signature = crypto.createHmac('sha256', secret).update(payload).digest('base64url');
  return `${payload}.${signature}`;
...
export function verifyVendorAdminSession(token: string | undefined, secret: string): boolean {
  ...
  try {
    ...
    return parsed.role === VENDOR_ADMIN_SESSION_ROLE && parsed.exp > Date.now();
  } catch {
    return false;
  }
}
