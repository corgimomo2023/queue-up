  ...
  const visible = useMemo(
    () =>
      events.filter(event => {
        ...
        return (
          (!queueId || event.queueId === queueId) && (!query || text.includes(query.toLowerCase()))
        );
      }),
  ...
  return (
    <>
      <header className="module-heading">
        ...
        <span className="status-pill">{t('audit.count', { count: visible.length })}</span>
      </header>
        ...
        <div className="audit-list">
          {visible.map(event => (
            <article key={event.id} className="audit-row">
              <div className="audit-icon">{event.action.charAt(0)}</div>
              <div>
                <strong>{titleCase(event.action)}</strong>
                <p>
                  {event.queueName || t('common.system')}
                  {event.customerId ? ` · ${t('audit.customer', { id: event.customerId })}` : ''}
                </p>
              </div>
              <span className="audit-actor">{titleCase(event.actorType)}</span>
              <time>{dateTime(event.createdAt, locale) || t('common.noActivity')}</time>
            </article>
          ))}
        </div>
        {visible.length === 0 && (
          <div className="empty">
            <strong>{t('audit.none')}</strong>
          </div>
        )}
      </Card>
    </>
