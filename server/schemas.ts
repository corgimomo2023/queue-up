...
export const staffPasswordSchema = z.string().min(8).max(128).regex(/\S/, 'Password must include a non-space character');

export const createQueueSchema = z.strictObject({
  ...
  if (value.startMode === 'scheduled' && !value.startLocal) {
    context.addIssue({ code: 'custom', message: 'Scheduled start is required', path: ['startLocal'] });
  }
});
...
export const joinSchema = z.strictObject({
  ...
  phone: z.string().trim().regex(/^\+?[0-9 ()-]{7,25}$/).optional().or(z.literal('')),
...
export const customerParamsSchema = z.strictObject({ queueId: z.string().min(1), customerId: z.coerce.number().int().positive() });
