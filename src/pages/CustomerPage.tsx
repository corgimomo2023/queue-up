...
export function CustomerPage() {
  ...
  useEffect(() => {
    ...
    source.addEventListener('queue_ended', () => { source.close(); void loadQueue(); });
    source.addEventListener('queue_archived', () => { source.close(); setQueue(null); setTicket(null); setError('errors.eventNotFound'); customerTicketStore.remove(queueId); });
  ...
  if (!queue) {
    return <Shell><div className="narrow"><Card>
      <p>{t('customer.loading')}</p><ErrorMessage message={error ? t(error) : ''} />
    </Card></div></Shell>;
  }

  if (queue.lifecycleStatus === 'scheduled') {
    return <Shell><div className="narrow"><Card className="lifecycle-card">
  ...
  return <Shell><div className="customer-wrap">
    <div className="queue-title">
      <EventBrand event={queue} eyebrow={t('customer.queue')} />
      <p className="muted">{t('customer.liveUpdates')}</p>
    </div>

    {ticket ? <Card className="ticket-card">
      <p className="eyebrow">{t('customer.position')}</p>
      <div className="ticket-number">
        {ticket.status === 'served' ? t('customer.now') : ticket.position}
      </div>
      <h2>{ticket.status === 'served'
        ? t('customer.yourTurn')
        : ticket.isNext ? t('customer.next') : t('customer.inLine')}</h2>
      {calledNotification && <div className="alert" role="status"><strong>{t('customer.called')}</strong></div>}

      {ticket.status === 'waiting' && <>
        <div className="queue-stats" aria-label={t('customer.statsLabel')}>
          <div className="queue-stat"><span>{t('customer.peopleAhead')}</span><strong>{ticket.peopleAhead}</strong></div>
          <div className="queue-stat"><span>{t('customer.totalWaiting')}</span><strong>{ticket.waitingCount}</strong></div>
        </div>
      ...
      >{t('customer.leave')}</button>
    </Card> : <Card>
      <div className="section-heading">
        <p className="eyebrow">{t('customer.join')}</p>
        <h2>{queue.waitingCount
          ? t('customer.waitingNow', { count: queue.waitingCount, unit: t(queue.waitingCount === 1 ? 'common.person' : 'common.people') })
          : t('customer.first')}</h2>
        <p className="muted">{t('customer.joinHint')}</p>
      </div>
      <form className="form-stack" onSubmit={join}>
        <label>{t('customer.name')}<input name="name" required maxLength={80} autoComplete="name" placeholder={t('customer.namePlaceholder')} /></label>
        <label>{t('customer.phone')} <span className="optional">{t('common.optional')}</span><input name="phone" type="tel" placeholder={t('customer.phonePlaceholder')} /></label>
        <label>{t('customer.message')} <span className="optional">{t('common.optional')}</span><textarea name="message" maxLength={200} rows={4} placeholder={t('customer.messagePlaceholder')} /></label>
        <ErrorMessage message={error ? t(error) : ''} />
        <button className="button primary" disabled={busy}>{busy ? t('customer.joining') : t('customer.joinButton')}</button>
      </form>
    </Card>}
  </div></Shell>;
}
