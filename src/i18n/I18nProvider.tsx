...
function TranslationBridge({ locale, setLocale, children }: PropsWithChildren<{ locale: AppLocale; setLocale: (locale: AppLocale) => void }>) {
  const { t: translate } = useTranslation();
  const value = useMemo(() => ({ locale, setLocale, t: translate as AppTranslate }), [locale, setLocale, translate]);
...
export function AppI18nProvider({ children }: PropsWithChildren) {
  ...
  return <I18nextProvider i18n={instance}>
    <TranslationBridge locale={locale} setLocale={setLocale}>{children}</TranslationBridge>
  </I18nextProvider>;
}
