...
describe('SuperAdminPage', () => {
  ...
  beforeEach(() => {
    vi.stubGlobal('fetch', vi.fn().mockImplementation(() => Promise.resolve(new Response(JSON.stringify({ error: 'Super admin authentication required' }), {
  ...
  it('keeps admin module names free of browser hyperlink styling', () => {
    ...
    expect(styles).toMatch(/\.admin-module-nav a\{[^}]*text-decoration:none/);
    expect(styles).toMatch(/\.admin-module-nav a:visited\{[^}]*color:#5d554f/);
  });
  ...
  it('uses module navigation and opens audit as its own workspace', async () => {
    vi.stubGlobal('fetch', vi.fn().mockImplementation((input: RequestInfo | URL) => {
      ...
      const body = url.includes('/audit')
        ? { events: [{ id: 1, queueId: null, queueName: null, customerId: null, actorType: 'super_admin', action: 'SUPER_ADMIN_LOGIN_SUCCESS', metadata: {}, ipAddress: null, userAgent: null, createdAt: '2026-07-22T09:00:00.000Z' }] }
        : { summary: { queues: 2, removedQueues: 0, scheduledQueues: 0, activeQueues: 2, endedQueues: 0, waiting: 3, served: 4, totalCustomers: 7 }, queues: [] };
      return Promise.resolve(new Response(JSON.stringify(body), { status: 200, headers: { 'content-type': 'application/json' } }));
    }));

    render(<MemoryRouter initialEntries={['/super-admin/overview']}><SuperAdminPage /></MemoryRouter>);
    expect(await screen.findByRole('heading', { name: 'Overview' })).toBeInTheDocument();
    const menu = screen.getByRole('button', { name: 'Modules' });
    fireEvent.click(menu);
    expect(screen.getByRole('button', { name: 'Close' })).toHaveAttribute('aria-expanded', 'true');
    fireEvent.click(screen.getByRole('button', { name: 'Close' }));
    expect(screen.getByRole('button', { name: 'Modules' })).toHaveAttribute('aria-expanded', 'false');
  ...
  it('hides dead Staff Admin and customer links for archived events', async () => {
    vi.stubGlobal('fetch', vi.fn().mockImplementation((input: RequestInfo | URL) => {
      ...
      const body = url.includes('/queues/archived-one')
        ? { queue: { queueId: 'archived-one', name: 'Archived Event', vendorUrl: '/vendor/archived-one', customerUrl: '/q/archived-one', startAt: '2026-07-22T08:00:00.000Z', endAt: null, lifecycleStatus: 'active', timeZone: 'Asia/Hong_Kong', isRemoved: true, removedAt: '2026-07-23T08:00:00.000Z' }, counts: { waiting: 0, served: 0, ended: 0, total: 0 }, customers: [] }
        : url.includes('/audit') ? { events: [] }
          : { summary: { queues: 0, removedQueues: 1, scheduledQueues: 0, activeQueues: 0, endedQueues: 0, waiting: 0, served: 0, totalCustomers: 0 }, queues: [] };
      return Promise.resolve(new Response(JSON.stringify(body), { status: 200, headers: { 'content-type': 'application/json' } }));
    }));

    render(<MemoryRouter initialEntries={['/super-admin/queues/archived-one']}><SuperAdminPage /></MemoryRouter>);
  ...
  it('keeps queue inspection and system status in separate modules', async () => {
    vi.stubGlobal('fetch', vi.fn().mockImplementation((input: RequestInfo | URL) => {
      ...
      else if (url.includes('/queues/test-one')) body = { queue: { queueId: 'test-one', name: 'Test One', vendorUrl: '/vendor/test-one', customerUrl: '/q/test-one', startAt: '2026-07-22T08:00:00.000Z', endAt: '2026-07-23T15:59:59.999Z', lifecycleStatus: 'active', timeZone: 'Asia/Hong_Kong', isRemoved: false, removedAt: null }, counts: { waiting: 1, served: 0, ended: 0, total: 1 }, customers: [{ customerId: 1, name: 'Alan', phoneMasked: '91••••67', message: 'Window seat', joinedAt: '2026-07-22T09:00:00.000Z', status: 'waiting', endedAt: null, endedReason: null }] };
      else body = { summary: { queues: 1, removedQueues: 0, scheduledQueues: 0, activeQueues: 1, endedQueues: 0, waiting: 1, served: 0, totalCustomers: 1 }, queues: [{ queueId: 'test-one', name: 'Test One', createdAt: '2026-07-22T08:00:00.000Z', waitingCount: 1, servedCount: 0, totalCustomers: 1, lastActivityAt: '2026-07-22T09:00:00.000Z', startAt: '2026-07-22T08:00:00.000Z', endAt: null, lifecycleStatus: 'active', timeZone: 'Asia/Hong_Kong', isRemoved: false, removedAt: null }] };
      return Promise.resolve(new Response(JSON.stringify(body), { status: 200, headers: { 'content-type': 'application/json' } }));
    }));
