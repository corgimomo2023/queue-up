...
describe('event branding and lifecycle UI', () => {
  ...
  it('shows shared branding while retaining active public queue actions', async () => {
    vi.stubGlobal('fetch', vi.fn(() => response({ ...brand, ...period, waitingCount: 0 })));
    render(<MemoryRouter initialEntries={['/q/summer-show']}><Routes><Route path="/q/:queueId" element={<CustomerPage />} /></Routes></MemoryRouter>);

    expect(await screen.findByRole('img', { name: 'Summer Show event logo' })).toHaveAttribute('src', brand.logoUrl);
  ...
  it('presents branded scheduled public state with exact HKT opening and no queue action', async () => {
    vi.stubGlobal('fetch', vi.fn(() => response({ ...brand, ...period, lifecycleStatus: 'scheduled', waitingCount: 0 })));
    render(<MemoryRouter initialEntries={['/q/summer-show']}><Routes><Route path="/q/:queueId" element={<CustomerPage />} /></Routes></MemoryRouter>);
  ...
  it('presents branded closed Staff state with the inclusive HKT end and no password or queue actions', async () => {
    vi.stubGlobal('fetch', vi.fn((input: RequestInfo | URL) => String(input).includes('/api/vendor/')
      ? response({ error: 'Vendor authentication required' }, 401)
      : response({ ...brand, ...period, lifecycleStatus: 'ended', waitingCount: 0 })));
    render(<MemoryRouter initialEntries={['/vendor/summer-show']}><Routes><Route path="/vendor/:queueId" element={<VendorPage />} /></Routes></MemoryRouter>);
  ...
  it('never flashes Staff login while the public scheduled lifecycle is still loading', async () => {
    ...
    const periodResponse = new Promise<Response>(resolve => { resolvePeriod = resolve; });
    const fetchMock = vi.fn((input: RequestInfo | URL) => String(input).includes('/api/vendor/')
      ? response({ error: 'Vendor authentication required' }, 401)
      : periodResponse);
    vi.stubGlobal('fetch', fetchMock);
    render(<MemoryRouter initialEntries={['/vendor/summer-show']}><Routes><Route path="/vendor/:queueId" element={<VendorPage />} /></Routes></MemoryRouter>);

    await waitFor(() => expect(fetchMock).toHaveBeenCalledWith('/api/vendor/summer-show', expect.anything()));
    expect(screen.queryByLabelText('Password')).not.toBeInTheDocument();
    resolvePeriod(await response({ ...brand, ...period, lifecycleStatus: 'scheduled', waitingCount: 0 }));
    expect(await screen.findByRole('heading', { name: 'Not open yet' })).toBeInTheDocument();
    expect(screen.queryByLabelText('Password')).not.toBeInTheDocument();
  });
  ...
  it('supports Page Setup description and raw logo upload during event creation', async () => {
    ...
    render(<MemoryRouter initialEntries={['/super-admin/queues']}><QueuesModule overview={overview} /></MemoryRouter>);
    ...
    fireEvent.change(screen.getByLabelText('Event description'), { target: { value: 'A neighbourhood celebration.' } });
    expect(screen.getByLabelText('Event description')).toHaveAttribute('maxlength', '500');
    fireEvent.change(screen.getByLabelText('Staff Admin password'), { target: { value: 'password-123' } });
    const logo = new File(['png'], 'autumn.png', { type: 'image/png' });
    fireEvent.change(screen.getByLabelText('Event logo'), { target: { files: [logo] } });
    expect(screen.getByRole('img', { name: 'Autumn Fair logo preview' })).toHaveAttribute('src', 'blob:preview');
    fireEvent.click(screen.getByRole('button', { name: 'Create event' }));

    await waitFor(() => expect(fetchMock).toHaveBeenCalledWith('/api/super-admin/queues/autumn-fair/logo', expect.anything()));
  });

  it('stages logo removal so Cancel sends no DELETE and Save sends it once', async () => {
    const detail = {
      queue: { ...brand, ...period, vendorUrl: '/vendor/summer-show', customerUrl: '/q/summer-show' },
      counts: { waiting: 0, served: 0, ended: 0, total: 0 },
      customers: [],
    };
    const fetchMock = vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
      if (init?.method === 'DELETE' && String(input).endsWith('/logo')) return response({ logoUrl: null });
      return response(detail);
    });
    vi.stubGlobal('fetch', fetchMock);

    render(<MemoryRouter initialEntries={['/super-admin/queues/summer-show']}><Routes><Route path="/super-admin/queues/:queueId" element={<QueuesModule overview={overview} />} /></Routes></MemoryRouter>);
    ...
    expect(fetchMock).not.toHaveBeenCalledWith('/api/super-admin/queues/summer-show/logo', expect.objectContaining({ method: 'DELETE' }));

    fireEvent.click(screen.getByRole('button', { name: 'Edit event' }));
    fireEvent.click(screen.getByRole('button', { name: 'Remove logo' }));
    fireEvent.click(screen.getByRole('button', { name: 'Save event' }));
    await waitFor(() => expect(fetchMock).toHaveBeenCalledWith('/api/super-admin/queues/summer-show/logo', expect.objectContaining({ method: 'DELETE' })));
  });

  it('navigates to the created event with a localized warning when only logo upload fails', async () => {
    const detail = {
      queue: { ...brand, queueId: 'autumn-fair', name: 'Autumn Fair', logoUrl: null, ...period, vendorUrl: '/vendor/autumn-fair', customerUrl: '/q/autumn-fair' },
      counts: { waiting: 0, served: 0, ended: 0, total: 0 }, customers: [],
    };
    ...
    render(<MemoryRouter initialEntries={['/super-admin/queues']}><Routes><Route path="/super-admin/queues/*" element={<QueuesModule overview={overview} />} /></Routes></MemoryRouter>);
    fireEvent.click(screen.getByRole('button', { name: 'Create event' }));
    fireEvent.change(screen.getByLabelText('Event name'), { target: { value: 'Autumn Fair' } });
    fireEvent.change(screen.getByLabelText('Staff Admin password'), { target: { value: 'password-123' } });
    fireEvent.change(screen.getByLabelText('Event logo'), { target: { files: [new File(['png'], 'autumn.png', { type: 'image/png' })] } });
    fireEvent.click(screen.getByRole('button', { name: 'Create event' }));

    expect(await screen.findByText('Event details were saved, but the logo upload failed. You can retry from Edit event.')).toBeInTheDocument();
  ...
  it('keeps archived Page Setup disabled and read-only', async () => {
    const detail = {
      queue: { ...brand, ...period, vendorUrl: '/vendor/summer-show', customerUrl: '/q/summer-show', isRemoved: true, removedAt: '2026-08-01T00:00:00.000Z' },
      counts: { waiting: 0, served: 0, ended: 0, total: 0 }, customers: [],
    };
    vi.stubGlobal('fetch', vi.fn(() => response(detail)));
    render(<MemoryRouter initialEntries={['/super-admin/queues/summer-show']}><Routes><Route path="/super-admin/queues/:queueId" element={<QueuesModule overview={overview} />} /></Routes></MemoryRouter>);
