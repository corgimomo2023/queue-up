...
const activePeriod = {
  startAt: '2026-07-01T00:00:00.000Z', endAt: null,
  lifecycleStatus: 'active', timeZone: 'Asia/Hong_Kong', isRemoved: false, removedAt: null,
};
...
describe('Staff Admin entry flow', () => {
  ...
  it('uses the landing page for Event ID and password login without exposing Vendor Admin access', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Password'), { target: { value: 'staff-password-123' } });
  ...
  it('shows the customer public URL in the Staff Admin dashboard', async () => {
    vi.stubGlobal('fetch', vi.fn((input: RequestInfo | URL) => String(input).includes('/api/vendor/')
      ? response({ queueId: 'megabox-summer', name: 'Summer Event', waitingCount: 0, customers: [], customerUrl: '/q/megabox-summer', ...activePeriod })
      : response({ queueId: 'megabox-summer', name: 'Summer Event', waitingCount: 0, ...activePeriod })));

    render(<MemoryRouter initialEntries={['/vendor/megabox-summer']}><Routes>
