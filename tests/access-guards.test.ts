...
describe('actor and event access boundaries', () => {
  ...
  it('keeps Vendor Admin and Staff Admin sessions in separate actor scopes', async () => {
    await request(context.app).get('/api/super-admin/overview').set('Cookie', firstStaffCookie).expect(401);
    await request(context.app).get(`/api/vendor/${firstEvent}`).set('Cookie', vendorCookie).expect(401);
  });

  it('binds Staff Admin sessions to exactly one event', async () => {
    await request(context.app).get(`/api/vendor/${secondEvent}`).set('Cookie', firstStaffCookie).expect(401);
    await request(context.app).post(`/api/vendor/${secondEvent}/serve-next`).set('Cookie', firstStaffCookie).expect(401);
  });
  ...
  it('binds customer tokens to exactly one event and actor action', async () => {
    ...
    await request(context.app)
      ...
      .expect(401);
    await request(context.app).get(`/api/queues/${secondEvent}/status`).query({ token }).expect(200);
  });
});
