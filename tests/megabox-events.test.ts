...
describe('MEGABOX event ownership and notification contract', () => {
  ...
  async function createEvent(overrides: Record<string, unknown> = {}) {
    ...
    const response = await request(context.app)
      .post('/api/queues')
      .set('Cookie', cookie)
      .send({ businessName: 'MEGABOX Summer Event', password: STAFF_PASSWORD, startMode: 'now', ...overrides })
      .expect(201);
  ...
  it('rejects moving an active event back to scheduled and keeps waiting tickets usable', async () => {
    const { cookie, eventId } = await createEvent();
    const joined = await request(context.app).post(`/api/queues/${eventId}/join`).send({ name: 'Waiting customer' }).expect(201);
  ...
  it('closes live customer streams and finalizes waiting tickets when an edit ends the event', async () => {
    const { cookie, eventId } = await createEvent({ startMode: 'scheduled', startLocal: '2026-07-01T09:00' });
    const joined = await request(context.app).post(`/api/queues/${eventId}/join`).send({ name: 'Waiting customer' }).expect(201);
    const server = context.app.listen();
    const address = server.address();
    if (!address || typeof address === 'string') throw new Error('listen failed');
    const response = await fetch(`http://127.0.0.1:${address.port}/api/queues/${eventId}/events?token=${joined.body.leaveToken}`);
    ...
    const ticket = context.db.prepare('SELECT status,ended_reason FROM customers WHERE id=?').get(joined.body.customerId) as Record<string, string>;
  ...
  it('emits only the queue.called event notification with the required five-minute message', async () => {
    ...
    await reader.cancel();
    await request(context.app).get(`/api/queues/${eventId}/events`).query({ token: joined.body.leaveToken }).expect(204);
    server.close();
  });
});
