...
describe('frontend API utilities', () => {
  ...
  it('parses JSON and throws safe API messages', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response(JSON.stringify({ queueId: 'abc' }), { status: 200, headers: { 'content-type': 'application/json' } })));
    await expect(apiRequest<{queueId:string}>('/api/x')).resolves.toEqual({ queueId: 'abc' });
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response(JSON.stringify({ error: 'Invalid request' }), { status: 400, headers: { 'content-type': 'application/json' } })));
