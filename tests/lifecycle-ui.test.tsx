...
const scheduledQueue = {
  queueId: 'later', name: 'Later Shop', waitingCount: 0,
  startAt: '2026-07-23T01:30:00.000Z', endAt: null,
  lifecycleStatus: 'scheduled', timeZone: 'Asia/Hong_Kong', isRemoved: false, removedAt: null,
};
...
describe('access period UI', () => {
  ...
  it('renders a non-interactive scheduled customer state', async () => {
    vi.stubGlobal('fetch', vi.fn(() => response(scheduledQueue)));
    render(<MemoryRouter initialEntries={['/q/later']}><Routes><Route path="/q/:queueId" element={<CustomerPage />} /></Routes></MemoryRouter>);
  ...
  it('renders scheduled Staff Admin state before asking for a password', async () => {
    ...
    render(<MemoryRouter initialEntries={['/vendor/later']}><Routes><Route path="/vendor/:queueId" element={<VendorPage />} /></Routes></MemoryRouter>);
    expect(await screen.findByRole('heading', { name: 'Not open yet' })).toBeInTheDocument();
